...
author: "José Enrique Manzanera López"
...
```{r}
commonAnimals <- intersect(df_mam, df_ani)
...
```{r}
differentAnimals <- setdiff(rownames(df_mam), rownames(df_ani))

differentAnimals


differentAnimals2 <- setdiff(rownames(df_ani), rownames(df_mam))
differentAnimals2
```
...
tail(df_mam["Arctic fox",])
tail(df_ani["Artic fox",])


tail(df_mam["Arctic ground squirrel",])
tail(df_ani["Artic ground squirrel",])


tail(df_mam["Mole rat",])
tail(df_ani["Mole",])

tail(df_mam["Patas monkey",])
tail(df_ani["Potar monkey",])
